...
function [r, c] = partial_corr_coef(S, i, j, Y)
    ...
    if isempty(Y)  % 处理 Y 为空的情况
        S2 = S(X,X);
    else
        if rcond(S(Y,Y)) < 1e-12  % 如果 S(Y,Y) 近似奇异
            warning('S(Y,Y) is nearly singular, using pseudo-inverse.');
            S2 = S(X,X) - S(X,Y) * pinv(S(Y,Y)) * S(Y,X);
        else
            S2 = S(X,X) - S(X,Y) * inv(S(Y,Y)) * S(Y,X);
        end
    end
   ...
   r = c / sqrt(S2(i2,i2) * S2(j2,j2));
    ...
    %if S2(i2,i2) <= 0 || S2(j2,j2) <= 0  % 避免 sqrt 计算负数
    %    warning('Non-positive variance encountered, setting r = NaN');
    %    r = NaN;
    %else
    %    r = c / sqrt(S2(i2,i2) * S2(j2,j2));
    %end
