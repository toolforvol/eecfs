function [MB,test,time] = EDMB_G2(Data,target,alpha,ns,p,maxK)
...
[MBOR_CPC,test2,~,~] = MBOR_PC_G2(Data,target,alpha,ns,p,maxK);
...
for i=1:length(PCselect)
    ...
    [pc_tmp,ntest2]=GetPC_G2(Data,PCselect(i),alpha, already_calculated_PCD, all_PCD, ns, p, maxK);
    %[pc_tmp,ntest2]=GetPCD_G2(Data,pc(i),alpha, ns, p, maxK);
     ...
     for j=1:length(pc_tmp)
         ...
         if isempty(find(PCselect==pc_tmp(j), 1))&& pc_tmp(j)~=target && isempty(find(dSep{pc_tmp(j)}==PCselect(i), 1))
             ...
             [pval]=my_g2_test(pc_tmp(j),target,[dSep{pc_tmp(j)},PCselect(i)],Data,ns,alpha);
