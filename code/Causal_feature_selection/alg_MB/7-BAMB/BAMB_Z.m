...
%logFile = fopen('BAMB_RNA_causal_structure_log.txt', 'w');
...
% === 统一输出最终 PC 中引出的所有配偶 ===
%printed_lines = {};  % 存储已经写入的完整字符串行

%for k = 1:length(pc) 
%    Y = pc(k);
%    final_spouses = spouse{1,Y};

%    for sp_idx = 1:length(final_spouses)
%        sp = final_spouses(sp_idx);
%
        % 构建将要写入的这一行内容
%        line_str = sprintf('[Spouse] Node %d is a final spouse of target %d via middle node %d\n', ...
%            sp, target, Y);

        % 如果还没写入过这行内容，则写入并记录
%        if ~any(strcmp(printed_lines, line_str))
%            fprintf(logFile, '%s', line_str);
%            printed_lines{end+1} = line_str;
%        end
%    end
%end
