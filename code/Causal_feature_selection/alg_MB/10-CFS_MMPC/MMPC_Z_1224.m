...
function [PC,test,time,sepset] = MMPC_Z_1224(Data,target,alpha,ns,p,maxK,all)
%
    ...
    for i=1:length(U)
        ...
        while length(PC) >= cutSetSize&&cutSetSize<=maxK
            ...
            for si=1:length(SS)
                ...
                [tmp_CI(X),dep(si)]=my_fisherz_test(X,target,Z,Data,ns,alpha);       
...
for i=(length(PC)-1):-1:1
    ...
    while length(CanPC) >= cutSetSize&&cutSetSize<=maxK
        ...
        for si=1:length(SS)
            ...
            [CI]=my_fisherz_test(X,target,Z,Data,ns,alpha);    
