...
def step_training(X_train, Y_train, out_importance, model_file):
    ...
    df = feature_imp
    df["Importance"] = (feature_imp.Value / feature_sum).round(4)
    df.to_csv(out_importance, index=False, sep=",")
    ...
    print(f"Average AUPR: {mean_aupr:.4f}\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--train_file", type=str, help="Input train file")
    parser.add_argument("--out_importance", type=str, help="Output feature importance file")
    ...
    step_training(train_data, Y_train, args.out_importance, args.model_file)